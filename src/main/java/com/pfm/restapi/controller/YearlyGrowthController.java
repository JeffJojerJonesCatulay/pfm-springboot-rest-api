...
public class YearlyGrowthController {
    ...
    @GetMapping("/search/yearlygrowth/allocId/{allocationId}")
    public ResponseEntity<Object> searchYearlyGrowthByAllocId(
        ...
        String endPoint = httpServletRequest.getServerName() + URL + "/search/yearlygrowth/allocId/" + allocationId;
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            List<YearlyGrowth> recordList = service.getYearlyGrowthByAllocId(allocationId);
            if (recordList.isEmpty()) {
                ...
                Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy).ascending());
                Page<YearlyGrowth> dataPage = new org.springframework.data.domain.PageImpl<>(recordList, pageable, recordList.size());
