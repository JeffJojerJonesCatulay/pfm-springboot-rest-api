...
public class WantListController {
    ...
    private static final Logger log = LoggerFactory.getLogger(WantListController.class);
    ...
    @GetMapping("/get/wantlist")
    public ResponseEntity<Object> getWantList(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/get/wantlist";
        try {
            ...
            Page<WantList> data = wantListService.getWantList(pageable);
    ...
    @GetMapping("/get/wantlist/id/{id}")
    public ResponseEntity<Object> getWantListById(@PathVariable Long id, HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/get/wantlist/id/" + id;
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            List<WantList> data = wantListService.getWantListById(id);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            response = Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, data);
    ...
    @PostMapping("/wantlist/create/")
    public ResponseEntity<Object> createWantList(@RequestBody WantList wantList, HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/wantlist/create/";
        try {
            tps.start(endPoint, " POST METHOD | " + wantList.getItem());
            log.debug("{} API - Start", endPoint);

            inputSanitation.sanitizeInput(wantList.getDateWanted());
            inputSanitation.sanitizeInput(wantList.getItem());
            inputSanitation.sanitizeInput(wantList.getAfford());
            inputSanitation.sanitizeInput(wantList.getRemarks());
            inputSanitation.sanitizeInput(wantList.getStatus());
            inputSanitation.sanitizeInput(wantList.getAddedBy());
            inputSanitation.sanitizeInput(wantList.getUpdateBy());

            WantList responses = wantListService.createWantList(wantList);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            ...
            String elapsedTime = tps.end(endPoint, " POST METHOD | " + wantList.getItem(), "HTTP STATUS: " + httpStatusReturn + " | STATUS : " + httpStatusMsgReturn);
            ...
            requestLogs.setRequestMethod(new Exception().getStackTrace()[0].getMethodName());
            requestLogs.setEndpoint(endPoint);
            requestLogs.setRequestDetails(wantList.toString());
            requestLogs.setRequestResponse(Objects.requireNonNull(body).toString());
            requestLogs.setStatusCode(Integer.parseInt(httpStatusReturn.replaceAll("\\D+", "")));
            requestLogs.setStatusResponse(httpStatusMsgReturn);
            requestLogs.setTimestamp((String) body.get("timestamp"));
            ...
            log.debug("POST METHOD | {} | HTTP STATUS: {} | STATUS : {}", wantList.getItem(), httpStatusReturn, httpStatusMsgReturn);
            log.debug("{} API - End", endPoint);
        }
    ...
    @PutMapping("/wantlist/update/{id}")
    public ResponseEntity<Object> updateWantList(@PathVariable Long id, @RequestBody WantList wantList, HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/wantlist/update/" + id;
        try {
            ...
            Optional<WantList> existing = wantListService.findById(id);
            if (existing.isEmpty()) {
                httpStatusReturn = String.valueOf(HttpStatus.BAD_REQUEST);
                httpStatusMsgReturn = Constant.GEN_ERR_MSG;
                response = Response.generateResponse(Constant.GEN_ERR_MSG, HttpStatus.BAD_REQUEST, null);
            } else {
                WantList responses = wantListService.updateWantList(wantList, id);
                httpStatusReturn = String.valueOf(HttpStatus.OK);
                httpStatusMsgReturn = Constant.SUCCESS;
                response = Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, responses);
            }
            ...
            requestLogs.setRequestMethod(new Exception().getStackTrace()[0].getMethodName());
            requestLogs.setEndpoint(endPoint);
            requestLogs.setRequestDetails(wantList.toString() + " PathVariable id: " + id);
            requestLogs.setRequestResponse(Objects.requireNonNull(body).toString());
            requestLogs.setStatusCode(Integer.parseInt(httpStatusReturn.replaceAll("\\D+", "")));
            requestLogs.setStatusResponse(httpStatusMsgReturn);
            requestLogs.setTimestamp((String) body.get("timestamp"));
    ...
    @DeleteMapping("/wantlist/delete/{id}")
    public ResponseEntity<Object> deleteWantList(@PathVariable Long id, HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/wantlist/delete/" + id;
        try {
            ...
            if (existing.isEmpty()) {
                ...
                wantListService.deleteWantList(id);
                httpStatusReturn = String.valueOf(HttpStatus.OK);
                httpStatusMsgReturn = Constant.SUCCESS;
    ...
    @GetMapping("/search/wantlist")
    public ResponseEntity<Object> searchWantList(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(required = false) String item,
            @RequestParam(required = false) String afford,
            @RequestParam(required = false) String remarks,
            @RequestParam(required = false) String status,
            HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/search/wantlist";
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            inputSanitation.validateSize(page);
            inputSanitation.validateSize(size);
            inputSanitation.sanitizeInput(item);
            inputSanitation.sanitizeInput(afford);
            ...
            wantList.setAfford(afford);
            wantList.setRemarks(remarks);
            wantList.setStatus(status);

            log.debug("WantList - Search Value: {} ", wantList.toString());

            Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy).ascending());
            Page<WantList> data = wantListService.findByCustomSearch(pageable, wantList);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            response = Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, data);
        } catch (BadCredentialsException | DataIntegrityViolationException | JwtException | IllegalArgumentException e) {
