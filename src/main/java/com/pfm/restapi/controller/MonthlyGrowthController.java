...
public class MonthlyGrowthController {
    ...
    public ResponseEntity<Object> getMonthlyGrowthByAllocId(@PathVariable Long id, HttpServletRequest httpServletRequest) {
        ...
        try {
            ...
            List<MonthlyGrowth> data = monthlyGrowthService.getMonthlyGrowthById(id);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            response = Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, data);
    ...
    @GetMapping("/search/monthlygrowth/allocId/{allocationId}")
    public ResponseEntity<Object> searchMonthlyGrowthByAllocId(
        ...
        String endPoint = httpServletRequest.getServerName() + URL + "/search/monthlygrowth/allocId/" + allocationId;
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            List<MonthlyGrowth> recordList = monthlyGrowthService.getMonthlyGrowthByAllocId(allocationId);
            if (recordList.isEmpty()) {
                httpStatusReturn = String.valueOf(HttpStatus.BAD_REQUEST);
                httpStatusMsgReturn = Constant.GEN_ERR_MSG;
                response = Response.generateResponse(Constant.GEN_ERR_MSG, HttpStatus.BAD_REQUEST, null);
            } else {
                List<MonthlyGrowth> filteredList = recordList.stream()
                    .filter(record -> month == null || record.getMonth() != null && record.getMonth().toLowerCase().contains(month.toLowerCase()))
                    .collect(java.util.stream.Collectors.toList());

                Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy).ascending());
                Page<MonthlyGrowth> dataPage = new org.springframework.data.domain.PageImpl<>(filteredList, pageable, filteredList.size());
