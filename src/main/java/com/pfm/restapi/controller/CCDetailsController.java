...
public class CCDetailsController {
    ...
    @GetMapping("/search/cc.details")
    public ResponseEntity<Object> searchCCDetails(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
            @RequestParam(defaultValue = "ccId") String sortBy,
            @RequestParam(required = false) String ccName,
            @RequestParam(required = false) String ccLastDigit,
            @RequestParam(required = false) String ccAcronym,
            HttpServletRequest httpServletRequest){
        String endPoint = httpServletRequest.getServerName() + URL + "/search/cc.details";
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            inputSanitation.validateSize(page);
            inputSanitation.validateSize(size);
            inputSanitation.sanitizeInput(ccName);
            inputSanitation.sanitizeInput(ccLastDigit);
            inputSanitation.sanitizeInput(ccAcronym);

            CCDetails ccDetails = new CCDetails();
            ccDetails.setCcName(ccName);
            ccDetails.setCcLastDigit(ccLastDigit);
            ccDetails.setCcAcronym(ccAcronym);

            log.debug("CCDetails - Search Value: {} ", ccDetails.toString());

            Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy).ascending());
            Page<CCDetails> data = ccDetailsService.findByCustomSearch(pageable, ccDetails);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            response =  Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, data);
        } catch (BadCredentialsException | DataIntegrityViolationException | JwtException | IllegalArgumentException e){
