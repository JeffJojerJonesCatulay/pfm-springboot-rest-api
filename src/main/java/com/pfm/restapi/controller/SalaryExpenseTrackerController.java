...
public class SalaryExpenseTrackerController {
    ...
    @GetMapping("/search/salaryexpensetracker")
    public ResponseEntity<Object> searchSalaryExpenseTracker(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            ...
            HttpServletRequest httpServletRequest) {
        String endPoint = httpServletRequest.getServerName() + URL + "/search/salaryexpensetracker";
        try {
            tps.start(endPoint, " GET METHOD");
            log.debug("{} API - Start", endPoint);
            ...
            SalaryExpenseTracker salaryExpenseTracker = new SalaryExpenseTracker();
            salaryExpenseTracker.setExpenseDescription(expenseDescription);
            salaryExpenseTracker.setExpenseType(expenseType);

            log.debug("SalaryExpenseTracker - Search Value: {} ", salaryExpenseTracker.toString());

            Pageable pageable = PageRequest.of(page, size, Sort.by(sortBy).ascending());
            Page<SalaryExpenseTracker> data = salaryExpenseTrackerService.findByCustomSearch(pageable, salaryExpenseTracker);
            httpStatusReturn = String.valueOf(HttpStatus.OK);
            httpStatusMsgReturn = Constant.SUCCESS;
            response = Response.generateResponse(Constant.SUCCESS, HttpStatus.OK, data);
        } catch (BadCredentialsException | DataIntegrityViolationException | JwtException | IllegalArgumentException e) {
